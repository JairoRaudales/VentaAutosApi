...
@api_view(['GET', 'POST'])
def tipo_auto_marca_list(request, id):
    tipo_auto = get_object_or_404(TipoAuto, id=id)
    if request.method == 'GET':
        marca = Marca.objects.filter(id_tipo_mascota=tipo_auto)
...
def tipo_auto_detail(request, id):
    try:
        tipo_auto = TipoAuto().objects.get(id=id)
    except TipoAuto().DoesNotExist:
        return JsonResponse({'message': 'The tipo auto does not exist....'}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        tipo_auto_serializer = TipoAutoSerializer(tipo_auto)
        return JsonResponse(tipo_serializer.data)

    elif request.method == 'PUT':
        tipo_auto_data = JSONParser().parse(request)
        tipo_auto_serializer = TipoAutoSerializer(tipo_auto, data=tipo_auto_data)
        if tipo_auto_serializer.is_valid():
            tipo_auto_serializer.save()
            return JsonResponse(tipo_auto_serializer.data)
        return JsonResponse(tipo_auto_serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    elif request.method == 'DELETE':
        tipo_auto.delete()
        return JsonResponse({'message': 'tipo auto was deleted successfully!'}, status=status.HTTP_204_NO_CONTENT)